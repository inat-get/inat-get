...
RSpec::describe INatGet::Data::DSL do
  ...
  it 'check ranges' do
    obj = Wrapper::new
    today = Date::today
    expect(obj.eval { time_range date: today }).to eq(today.to_time ... (today + 1).to_time)
    start = Date::new(1901, 1, 1).to_time
    finish = Date::new(2001, 1, 1).to_time
    expect(obj.eval { time_range century: 20 }).to eq(start ... finish)
    start = Date::new(2026, 4, 1).to_time
    finish = Date::new(2026, 7, 1).to_time
    expect(obj.eval { time_range year: 2026, quarter: 2 }).to eq(start ... finish)
    start = Date::new(2025, 12, 1).to_time
    finish = Date::new(2026, 3, 1).to_time
